...
async function find(fn, options) {
  ...
  this.on('readable', wake);
  const cleanup = eos(this, { writable: false }, (err) => {
    error = err ? aggregateTwoErrors(error, err) : null;
    wake();
  });

  async function consumeChunk(chunk, source, currentIndex) {
    try {
      let v = fn(chunk, options);
      if (v && typeof v.then === 'function') {
        v = await v;
      }
      if (v && (foundIndex === undefined || foundIndex > currentIndex)) {
        foundIndex = currentIndex;
        result = chunk;
      }
    } catch (err) {
      destroyImpl.destroyer(source, err);
    } finally {
      currentConcurrency--;
      wake();
    }
  ...
  try {
    while (true) {
      ...
      if (currentConcurrency < concurrency && foundIndex === undefined) {
        const chunk = this.destroyed ? null : this.read();

        if (chunk !== null) {
          currentConcurrency++;
          void consumeChunk(chunk, this, sequence++);
          continue;
        }
      }
      ...
      if (currentConcurrency === 0 && (foundIndex !== undefined || error === null)) {
        break;
      }

      const pr = PromiseWithResolvers();
      continueLoop = pr.resolve;
      await pr.promise;
    }
  } catch (err) {
    destroyImpl.destroyer(this, err);
    ...
    this.off('readable', wake);
    cleanup();
  }

  if (destroyOnReturn && !this.errored) {
